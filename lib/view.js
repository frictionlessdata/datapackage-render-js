...
export function getResourceCachedValues(resource, rowsAsObjects = false) {
  let fieldNames
  if (resource.schema) {
    fieldNames = resource.schema.fields.map(field => {
      return field.name
    })
  }
  if (rowsAsObjects) {
...
export function handsOnTableToHandsOnTable(view) {
  ...
  if (view.resources[0].schema) {
    headers = view.resources[0].schema.fields.map(field => field.title || field.name)
    ...
    if (isArray(view.resources[0]._values[0])) {
      headers = view.resources[0]._values[0]
    } else if (isPlainObject(view.resources[0]._values[0])) {
